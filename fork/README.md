...
服务端的流程：  
   - 主进程接受客户端的连接请求  
   - 将新创建的套接字传给子进程，主进程继续等待连接请求  
   - 子进程与客户端进行交互，交互结束由 sigaction 结束子进程，释放资源  

因为子进程会复制主进程拥有的所有资源，因此不需要特地的传递套接字描述符，但是复制套接字之后，同一个套接字接口就会对于多个套接字。如主进程同时连接着服务端和客户端的套接字接口，子进程也是如此，这样子的话客户端就会连接到两个进程，要释放这个套接字的话必须关闭两个套接字描述符才行，因此[server.cpp](server.cpp)的101、123行分别关闭了主进程与客户端、子进程与服务端的套接字连接，这样就能始终保持服务端套接字只与主进程相连接，客户端套接字只与子进程连接。
...
### 多进程客户端
[mpclient.cpp](client.cpp):多进程客户端  

此前的回声客户端发送数据之后，会一直等待服务端的回声之后，才能继续发送数据。因为代码重复调用了 write/read 会形成阻塞，在一个进程内同时实现数据收发逻辑需要考虑更多细节。那我们可不可以分割数据收发过程，一个进程发数据，另一个进程收数据，这样就不用老是等待了。事实上这是可以的，分割 I/O 程序可以提高数据交换的性能。

> 《TCP/IP 网络编程》（ [韩] 尹圣雨  著 金国哲 译）C10.5 P10   
   实际上客户端没有特殊原因进行 I/O 分割，若进行反而使得程序更复杂，只是为了讲解 I/O 分割的方法，才举这个例子。
...
2. 多进程模型的优缺点？
   ...
   在克隆进程的时候，子进程会拥有独立的内存空间，完全和父进程独立开来，父、子进程拥有各自的数据区、堆区域、栈区域  

   优点：  
      - 可以满足程序并发的需求（狗头微笑）

   缺点：  
      - 创建进程的过程会带来一点开销（时间和空间）
      - 进程间通信（进行数据交换）需要特殊的 IPC 技术  
      - 最严重的缺点：频繁的上下文切换

3. 什么是线程（thread）？

   为了解决保持多进程的优点，同时在一定程度上改良其缺点，就诞生了线程。多线程之间享有共同的数据区、堆区域，各自独立的是栈区域，既获得了并发的能力，也降低了进程的开销。  
...
3. 进程和线程的异同？
   ...
   - 空间独立性：多进程各自空间独立，多线程共用数据区、堆区域  
   - 上下文切换开销：CPU 为了让程序都能在 CPU 上跑，会每隔一段时间切换一个程序，轮流跑，线程上下文切换开销比进程小，因为不需要切换数据区和堆      
   - 一个进程可以包含多个线程。
